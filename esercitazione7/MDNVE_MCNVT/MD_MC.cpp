...
// distance i-j in pbc
      ...
      if(dr < rcut)
      {
        g_r[int(dr/rcut*100)]+=2;
        vij = 1.0/pow(dr,12) - 1.0/pow(dr,6);
        v += vij;
        enp += vij + 0.5/pow(dr,6);
        count++;
      }
    }          
  }

  ofstream epot_inst("epot_inst.dat",ios::app);
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    G_r.open("output_gr.dat",ios::app);
    
    stima_pot = blk_av[iv]/blk_norm/(double)npart - 16 * M_PI * rho / (9 * pow(rcut,3)); //Potential energy
    ...
    stima_pres = blk_av[ip]/blk_norm - 32 * M_PI * rho *(1/(9*pow(rcut,9))-1/(6*pow(rcut,3))); //Pressure
...
//G(r)
    for(int i=0;i<100;i++){
      g_r[i]/=(rho*nstep*4*M_PI/3*(pow(rcut*(i+1)/100,3)-pow(rcut*i/100,3)));
      g_r_av[i]+=g_r[i];
      g_r_av2[i]+=g_r[i]*g_r[i];
      if(i<99) G_r << rcut*i/100 << setw(wd);
      else G_r << rcut*i/100 << endl;
    }
    for(int i=0;i<100;i++){
      G_r << g_r[i] << setw(wd);
    }
...
void ConfFinal(void)
{
  ofstream WriteConf, WriteVelocity, WriteSeed, G_r_av;

  cout << "Print final configuration to file config.out" << endl << endl;
  WriteConf.open("config.out");
  WriteVelocity.open("velocity.out");
  G_r_av.open("g(r)_average.dat");
  for(int i=0;i<100;i++){
    G_r_av << i*rcut/100 << setw(12) << g_r_av[i]/nblk << setw(12) << Error(g_r_av[i],g_r_av2[i],nblk) << endl;
  }
  for (int i=0; i<npart; ++i)
  {
